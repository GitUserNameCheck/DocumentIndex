...
from app.services.report_service import s3_delete_report, s3_upload_report
...
async def pager_process_document(document: Document, user_data: UserData, db: Session):
    ...
    try:
        ...
        files = {
            "file": (
                f"{document.s3_filename}.{document.s3_mime_type}",  # filename
                s3_client.get_object(Bucket=AWS_BUCKET, Key=f"documents/{document.s3_filename}.{document.s3_mime_type}")["Body"],
                f"application/{document.s3_mime_type}"              # content type (e.g. application/pdf)
            )
        }
        ...
        async with httpx.AsyncClient(timeout=120.0) as client:
            response = await client.post("http://localhost:8000/", data=data, files=files)
